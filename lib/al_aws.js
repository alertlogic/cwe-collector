...
var selfUpdate = function (callback) {
    ...
    lambda.updateFunctionCode(params, function(err, data) {
        if (err) {
            ...
            console.info('Lambda self-update successful.  Data: ' + JSON.stringify(data));
        }
        return callback(err);
    });
};
...
var writeKinesisRecord = function (kinesisArn, partitionKey, record) {
    var params = {
        Data: record,
        StreamName: arnToName(kinesisArn),
        PartitionKey: partitionKey
    };
    var kinesis = new AWS.Kinesis();
    kinesis.putRecord(params, function(err, resp) {
        if (err) {
...
module.exports = {
    ...
    arnToName : arnToName,
    writeKinesisRecord : writeKinesisRecord
};
